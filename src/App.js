...
export default function App() {
  ...
  const [latestDataTimestamp, setLatestDataTimestamp] =
    useState("not available yet");
  ...
  useEffect(() => {
    ...
    dataQueryUrl =
      dataQueryUrl +
      "&solved_percent_threshold=" +
      (minSolvedPercent.length > 0 ? minSolvedPercent : "100");
    dataQueryUrl =
      dataQueryUrl +
      "&3bv_threshold=" +
      (minBoard3bv.length > 0 ? minBoard3bv : "0");
    dataQueryUrl =
      dataQueryUrl +
      "&efficiency_threshold=" +
      (minEfficiency.length > 0 ? minEfficiency : "0");
    if (earliestDate !== "") {
    ...
    const getAndSetGameDataAndTimestamp = async () => {
      ...
      try {
        ...
        const timestampData = (
          await axios.get(
            "https://kennypeng15.pythonanywhere.com/latest-timestamp"
          )
        ).data;
        ...
        if (err.response) {
          // non-2xx status code
          alert(
            "Request for data failed - status code did not indicate success: " +
              err.response.data +
              ", " +
              err.response.status
          );
          ...
          alert(
            "Request for data failed - no response was received: " + err.request
          );
        } else {
    ...
    <>
      <div className={darkMode ? "app-wrapper-dark" : "app-wrapper"}>
        ...
        <div className="latest-timestamp">
          <p>Date of latest available game data: {latestDataTimestamp}</p>
        </div>
        <hr />
        <div className="filter-container">
            ...
            <div className="filter-radio">
              <p>
                <label>
                  <input
                    type="radio"
                    name="solvedOnlyRadio"
                    value={true}
                    defaultChecked={true}
                    onChange={() => setSolvedOnly(true)}
                  />{" "}
                  Yes
                </label>
              </p>
          ...
          {!solvedOnly && (
            <div className="filter">
              ...
              <div className="filter-input">
                <input
                  name="minSolvedPercentInput"
                  defaultValue={minSolvedPercent}
                  ref={minSolvedPercentRef}
                />
                <button
                  onClick={() => {
                    if (
                      parseInt(minSolvedPercentRef.current.value) &&
                      parseInt(minSolvedPercentRef.current.value) >= 50
                    ) {
                      setMinSolvedPercent(
                        minSolvedPercentRef.current.value.trim()
                      );
                    } else {
                      alert(
                        "Invalid filter input: solved percentage must be a number greater than or equal to 50."
                      );
                    }
                  }}
                >
            ...
            <div className="filter-input">
              ...
              <button
                onClick={() => {
                  ...
                  if (
                    minBoard3bvRef.current.value.trim() === "0" ||
                    (parseInt(minBoard3bvRef.current.value) &&
                      parseInt(minBoard3bvRef.current.value) >= 0)
                  ) {
                    setMinBoard3bv(minBoard3bvRef.current.value.trim());
                  } else {
                    alert(
                      "Invalid filter input: minimum board 3bv must be a number greater than or equal to 0."
                    );
                  }
                }}
              >
          ...
          <div className="filter">
            ...
            <div className="filter-input">
              <input
                name="minEfficiencyInput"
                defaultValue="0"
                ref={minEfficiencyRef}
              />
              <button
                onClick={() => {
                  if (
                    minEfficiencyRef.current.value.trim() === "0" ||
                    (parseInt(minEfficiencyRef.current.value) &&
                      parseInt(minEfficiencyRef.current.value) >= 0)
                  ) {
                    setMinEfficiency(minEfficiencyRef.current.value.trim());
                  } else {
                    alert(
                      "Invalid filter input: minimum efficiency must be a number greater than or equal to 0."
                    );
                  }
                }}
              >
          ...
          <div className="filter">
            <div className={darkMode ? "filter-title-dark" : "filter-title"}>
              Earliest date (YYYY-MM-DD; current:{" "}
              {earliestDate === "" ? "N/A" : earliestDate}):
            </div>
            <div className="filter-input">
              <input
                name="earliestDateInput"
                defaultValue=""
                ref={earliestDateRef}
              />
              <button
                onClick={() => {
                  if (
                    moment(
                      earliestDateRef.current.value.trim(),
                      "YYYY-MM-DD",
                      true
                    ).isValid()
                  ) {
                    ...
                    alert(
                      "Invalid filter input: earliest date must be a valid date in the format YYYY-MM-DD."
                    );
                  }
                }}
              >
          ...
          <div className="filter">
            <div className={darkMode ? "filter-title-dark" : "filter-title"}>
              Latest date (YYYY-MM-DD; current:{" "}
              {latestDate === "" ? "N/A" : latestDate}):
            </div>
            <div className="filter-input">
              <input
                name="latestDateInput"
                defaultValue=""
                ref={latestDateRef}
              />
              <button
                onClick={() => {
                  if (
                    moment(
                      latestDateRef.current.value.trim(),
                      "YYYY-MM-DD",
                      true
                    ).isValid()
                  ) {
                    ...
                    alert(
                      "Invalid filter input: latest date must be a valid date in the format YYYY-MM-DD."
                    );
                  }
                }}
              >
        ...
        {!loading && dataList.length > 0 && (
          <Graph dataList={dataList} darkMode={darkMode} />
        )}
        {!loading && dataList.length > 0 && (
          <StatisticsContainer dataList={dataList} darkMode={darkMode} />
        )}
        {!solvedOnly && (
          <div className="solved-only-disclaimer">
            Note: for unsolved games, the estimated time is used for all
        ...
        <div className="toggle-wrapper">
          <Toggle
            ...
            icons={{
              checked: (
                <div style={{ paddingTop: "4px" }} role="img" aria-label="sun">
                  🌙
                </div>
              ),
              unchecked: (

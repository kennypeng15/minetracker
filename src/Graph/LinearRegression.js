export default function LinearRegression(dataList) {
    ...
    lr["timeR2"] = Math.pow((n * sum_xy - sum_x * sum_y) / Math.sqrt((n * sum_xx - sum_x * sum_x) * (n * sum_yy - sum_y * sum_y)), 2);
    ...
    lr["3bvpsSlope"] = (n * sum_xy - sum_x * sum_y) / (n * sum_xx - sum_x * sum_x);
    lr["3bvpsIntercept"] = (sum_y - lr["3bvpsSlope"] * sum_x) / n;
    lr["3bvpsR2"] = Math.pow((n * sum_xy - sum_x * sum_y) / Math.sqrt((n * sum_xx - sum_x * sum_x) * (n * sum_yy - sum_y * sum_y)), 2);
    ...
    lr["efficiencySlope"] = (n * sum_xy - sum_x * sum_y) / (n * sum_xx - sum_x * sum_x);
    lr["efficiencyIntercept"] = (sum_y - lr["efficiencySlope"] * sum_x) / n;
    lr["efficiencyR2"] = Math.pow((n * sum_xy - sum_x * sum_y) / Math.sqrt((n * sum_xx - sum_x * sum_x) * (n * sum_yy - sum_y * sum_y)), 2);
    ...
    const maxPoint = dataList.length > 0 ? sorted[dataList.length - 1].epochValue + 500000000 : 0;
